...
class res_config_settings(osv.osv_memory, res_config_module_installation_mixin):
    ...
    def onchange_module(self, cr, uid, ids, field_value, module_name, context=None):
        ...
        module_ids = module_pool.search(
            ...
            ('state','in', ['to install', 'installed', 'to upgrade'])],
            context=context)
        module_ids_install = module_pool.search(
            cr, uid, [('name', '=', module_name.replace("module_", '')),
        ...
        if module_ids_install and field_value:
            dep_ids = module_pool.uninstalled_module_dependencies(cr, uid, module_ids_install, context=context)
            dep_name = [x['display_name'] for x  in module_pool.search_read(
                cr, uid, ['|', ('id','=',module_ids_install),('id','in', dep_ids),('application', '=', 'True')], context=context)]
            if dep_name:
                message = '\n'.join(dep_name)
                return {
                    'warning': {
                        'title': _('Warning!'),
                        'message': _('Enabling this feature you will install paid application \n%s') % message,
                    }
                }
        return {}
