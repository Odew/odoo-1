...
var GanttView = View.extend({
	...
	gantt.config.initial_scroll = false;
        gantt.config.columns = [{name:"text", label:_lt("Gantt View"), tree:true, width:'*' }];
	...
	if (group_bys[0] !== "_pseudo_group_by"){
	    new Model(self.dataset.model, self.last_contexts, self.last_domains)
		.query(['name'])
	...
	self.open_task_id = task_id;
        ...
        form_controller.on("load_record", self, function() {
            ...
            var button_edit = _.str.sprintf("<button class='oe_button oe_form_button_edit oe_bold oe_highlight'><span> %s </span></button>",_lt("Edit"));
            var button_save = _.str.sprintf("<button class='oe_button oe_form_button_save oe_bold oe_highlight'><span> %s </span></button>",_lt("Save"));
	    //Delete
	    var button_delete = _.str.sprintf("<button class='oe_button oe_form_button_delete delme'><span> %s </span></button>", _lt("Delete"));
		...
		if (confirm(_lt("Are you sure you want to delete this record"))) {
	...
	if (task.baseline_start && task.baseline_stop) {
	    ...
	    var start = time.auto_str_to_date(task.baseline_start);
	    var stop = time.auto_str_to_date(task.baseline_stop);
