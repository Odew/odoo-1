...
class TestResource(TestResourceCommon):
    ...
    def test_00_intervals(self):
        ...
        cleaned_intervals = self.calendar.interval_clean(intervals)
        ...
        result = self.calendar.interval_remove_leaves(working_interval, intervals)
        ...
        result = self.calendar.interval_schedule_hours(cleaned_intervals, 5.5)
        ...
        result = self.calendar.interval_schedule_hours(cleaned_intervals, 5.5, remove_at_end=False)
        self.assertEqual(len(result), 2, 'resource_calendar: wrong hours scheduling in interval')
        # First interval: 03, 8-10 untouches
    ...
    def test_10_calendar_basics(self):
        ...
        weekdays = self.calendar.get_weekdays(self.calendar)
    ...
    def test_40_calendar_hours_scheduling(self):
        ...
        res = self.calendar._interval_hours_get(
            ...
            resource_id=self.resource1.id, exclude_leaves=True)
        ...
        res = self.calendar.get_working_hours(
            self.date1.replace(hour=6, minute=0),
            self.date2.replace(hour=23, minute=0) + relativedelta(days=7),
            compute_leaves=False, resource_id=self.resource1.id)
        ...
        res = self.calendar._interval_hours_get(
            self.date1.replace(hour=6, minute=0),
            self.date2.replace(hour=23, minute=0) + relativedelta(days=7),
            resource_id=self.resource1.id, exclude_leaves=False)
        ...
        res = self.calendar.get_working_hours(
            self.date1.replace(hour=6, minute=0),
            self.date2.replace(hour=23, minute=0) + relativedelta(days=7),
            compute_leaves=True, resource_id=self.resource1.id)
        ...
        res = self.ResourceCalendar.get_working_hours(
            self.date1.replace(hour=6, minute=0),
    ...
    def test_50_calendar_schedule_days(self):
        ...
        res = self.ResourceCalendar.schedule_days_get_date(5, day_date=self.date1, default_interval=(8, 16))
