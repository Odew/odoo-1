...
class CrmClaim(models.Model):
    ...
    def onchange_partner_id(self):
        (self.email_from, self.partner_phone) = (self.partner_id.email, self.partner_id.phone)

    @api.model
    def create(self, vals):
        return super(CrmClaim, self.with_context(default_team_id=vals.get('team_id')) if vals.get('team_id') and not self.env.context.get('default_team_id') else self).create(vals)
    ...
    def message_new(self, msg, custom_values=None):
        ...
        """
        custom_values = {} if not custom_values else custom_values
...
class ResPartner(models.Model):
    ...
    def _compute_claim_count(self):
        count = dict([(result['partner_id'][0], result['partner_id_count']) for result in self.env['crm.claim'].read_group([('partner_id', 'in', self.ids)], ['partner_id'], ['partner_id'])])
        for partner in self:
            partner.claim_count = count.get(partner.id, 0)
