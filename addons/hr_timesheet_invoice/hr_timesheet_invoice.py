                ...
                for account in analytic_account_obj.browse(cr, uid, list(account_ids), context=context):
                    ...
                    for product_id, user_id, factor_id, total_price, qty, uom in cr.fetchall():
                        ...
                        cr.execute("SELECT * FROM account_analytic_line \
                               WHERE account_id = %s and id IN %s AND product_id=%s and to_invoice=%s \
                               ORDER BY account_analytic_line.date", (account.id, tuple(ids), product_id, factor_id))
                        ...
                        cr.execute("UPDATE account_analytic_line \
                               SET invoice_id=%s \
                               WHERE account_id = %s and id IN %s", (last_invoice, account.id, tuple(ids)))
