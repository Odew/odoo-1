    ...
    def default_get(self, fields):
        ...
        if active_ids:
            ...
            if default_composition_mode == 'mass_mail':
                partner_assigned_ids = CrmLead.search_geo_partner(active_ids)
            else:
                partner_assigned_ids = dict((lead.id, lead.partner_assigned_id and lead.partner_assigned_id.id or False) for lead in leads)
...
class CrmLeadAssignation(models.TransientModel):
    ...
    def on_change_partner_assigned_id(self):
        if not self.partner_assigned_id:
            return {'value': {'lead_location': False}}
        partner = self.env['res.partner'].browse(self.partner_assigned_id.id)
        partner_location = []
        if partner.country_id:
            partner_location.append(partner.country_id.name)
        if partner.city:
