...
class links(models.Model):
    """ Links allow users to wrap any URL into a short and trackable URL
    via a frontend and backend interface. Llinks counts clicks on each tracked link.
    ...
    link_click_ids = fields.One2many('links.click', 'link_id', string='Clicks')
    title = fields.Char(string='Page Title', store=True)
    favicon = fields.Char(string='Favicon', compute='_compute_favicon', store=True)
    link_code_ids = fields.One2many('links.code', 'link_id', string='Codes')
    ...
    def _compute_code(self):
        record = self.env['links.code'].search([('link_id', '=', self.id)], limit=1, order='id DESC')
    ...
    def action_view_statistics(self):
        action = self.env['ir.actions.act_window'].for_xml_id('links', 'action_view_click_statistics')
    ...
    def create(self, vals):
        ...
        link = super(links, self).create(create_vals)

        code = self.env['links.code'].get_random_code_string()
        self.env['links.code'].create({'code': code, 'link_id': link.id})

        return link

    @api.model
    def get_url_from_code(self, code, context=None):
        code_rec = self.env['links.code'].sudo().search([('code', '=', code)])
...
class links_code(models.Model):
    _name = "links.code"

    code = fields.Char(string='Short URL Code', store=True)
    link_id = fields.Many2one('links', 'Link', required=True, ondelete='cascade')
...
class links_click(models.Model):
    ...
    def add_click(self, code, ip, country_code, stat_id=False):
        self = self.sudo()
        code_rec = self.env['links.code'].search([('code', '=', code)])
