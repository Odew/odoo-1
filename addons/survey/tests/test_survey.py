...
class TestSurvey(TransactionCase):
    ...
    def setUp(self):
        ...
        (self.Survey, self.Question, self.UserInputLine) = (self.env['survey.survey'], self.env['survey.question'], self.env['survey.user_input_line'])
        (self.UserInput, self.Stage, self.SurveyMailMessage) = (self.env['survey.user_input'], self.env['survey.stage'], self.env['survey.mail.compose.message'])
    ...
    def test_00_create_minimal_survey(self):
        ...
        self.assertEqual(len(self.page1.question_ids), 1, msg="Additioanl questions are linked with the page afer creation.")
    ...
    def test_01_question_type_validation_save_line_function(self):
        for (type, text) in self.Question._columns['type'].selection:
            # Each Type must have validation function.
            self.assertTrue(hasattr(self.Question, 'validate_' + type), msg="Survey Question must have a validation method in\
                the form of 'validate_' followed by the name of the type.")

            # Survey Input Lines must have validation function for each line.
            self.assertTrue(hasattr(self.UserInputLine, 'save_line_' + type), msg="Survey Inputline must have Save method in \
                the form of 'save_line_' followed by the name of the type.")

    def test_02_question_answer_required(self):
        for (type, text) in self.Question._columns['type'].selection:
            # Blank value of field is not accepted when mandatory is positive.
            if type == 'multiple_choice':
                ...
                question = self.Question.sudo(self.survey_manager).create({
                    'page_id': self.page1.id, 'question': 'Q0', 'type': type, 'constr_mandatory': True, 'constr_error_msg': 'Error'})
            answer_tag = '%s_%s_%s' % (self.survey1.id, self.page1.id, question.id)
            self.assertDictEqual({answer_tag: "Error"}, self.Question.validate_question(question, {answer_tag: ''}, answer_tag), msg="\
                Validation function for type '" + type + "' is unable to generate error if it is mendetory and answer is blank.")
    ...
    def test_03_question_textbox(self):
        ...
        for i in range(len(questions)):
            ...
            self.assertEqual(self.Question.validate_question(questions[i], {answer_tag: results[i][0]}, answer_tag), {answer_tag: results[i][1]}, msg="\
                Validation function for textbox is unable to notify if answer is vaiolating the validation rules")
    ...
    def test_04_question_numerical_box(self):
        ...
        for i in range(len(results)):
            self.assertEqual(self.Question.validate_question(question, {answer_tag: results[i][0]}, answer_tag), {answer_tag: results[i][1]}, msg="\
                Validation function for type numerical_box is unable to notify if answer is vaiolating the validation rules")
    ...
    def test_05_question_datetime(self):
        ...
        for i in range(len(results)):
            self.assertEqual(self.Question.validate_question(question, {answer_tag: results[i][0]}, answer_tag), {answer_tag: results[i][1]}, msg="\
                Validation function for type datetime is unable to notify if answer is vaiolating the validation rules")
    ...
    def test_08_survey_urls(self):
        ...
        for urltype, urltxt in urltypes.iteritems():
            ...
            if urltype == 'public':
                ...
                self.assertEqual(url_html % full_url, getattr(self.survey1, urltype + '_url_html'), msg="Public url html is not according \
                  to expectations.")
                self.assertEqual(url_html % ('/' + url), getattr(self.survey1.with_context({'relative_url': True}), urltype + '_url_html'),  msg="\
                    Public url html is not according to expectations.")
    ...
    def test_10_survey_result_simple_multiple_choice(self):
        ...
        answers.sort()
        prp_result.sort()
        self.assertEqual(prp_result, answers, msg="Statistics of simple, multiple choice questions are differe from expectation")
    ...
    def test_12_survey_result_numeric_box(self):
        ...
        exresult = {
            ...
            'min': round(min(num), 2), 'sum': nsum, 'most_comman': Counter(num).most_common(5)}
