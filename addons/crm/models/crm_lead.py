...
class CrmLead(format_address, models.Model):
    ...
    def get_empty_list_help(self, help):
        ...
        ctx['empty_list_help_document_name'] = _("leads")
    ...
    def _get_default_team_id(self, user_id=False):
        ...
        return self._resolve_team_id_from_context()
    ...
    def _calls_count(self):
        phonecall_data = self.env['crm.phonecall'].read_group([('opportunity_id', 'in', self.ids)], ['opportunity_id'], ['opportunity_id'])
    ...
    email_from = fields.Char(
        string='Email', size=128, help="Email address of the contact", index=True)
    ...
    partner_name = fields.Char(
        string="Customer Name", help='The name of the future partner company that will be created while converting the lead into opportunity', index=True)
    opt_out = fields.Boolean(string='Opt-Out', oldname='optout',
                             help="If opt-out is checked, this contact has refused to receive emails for mass mailing and marketing campaign. "
                             "Filter 'Available for Mass Mailing' allows users to filter the leads when performing mass mailing.")
    type = fields.Selection(
        [('lead', 'Lead'), ('opportunity', 'Opportunity')],
        index=True, help="Type is used to separate Leads and Opportunities", default='lead')
    priority = fields.Selection(
        crm.AVAILABLE_PRIORITIES, index=True,
    ...
    date_last_stage_update = fields.Datetime(string='Last Stage Update', index=True,
    ...
    def referencable_models(self):
        request_link = self.env['res.request.link'].search([])
        return [(r.object, r.name) for r in request_link]
    ...
    def on_change_user(self):
        ...
        if self.user_id and not team_id and self.user_id.has_group('base.group_multi_salesteams'):
            team = self.env['crm.team'].search(['|', ('user_id', '=', self.user_id.id), ('member_ids', '=', self.user_id.id)], limit=1)
            if team:
                team_id = team.id
    ...
    def merge_opportunity(self, user_id=False, team_id=False):
        ...
        fields = list(CRM_LEAD_FIELDS_TO_MERGE)
        merged_data = opportunities._merge_data(fields)
        if user_id:
    ...
    def _create_lead_partner(self):
        ...
        partner_id = contact_id or partner_company_id or self._lead_create_contact(lead.name, False)
        return partner_id       
    ...
    def message_get_reply_to(self):
        ...
        return dict((lead.id, aliases.get(lead.team_id.id, False)) for lead in self.sudo())
