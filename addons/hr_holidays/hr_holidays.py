...
class hr_employee(osv.Model):
    ...
    def _leaves_count(self, cr, uid, ids, fields, arg, context=None):
        ...
        if 'leaves_count' in fields:
            ...
            if leaves:
                res.update(dict([(leave['employee_id'][0], {'leaves_count': leave['number_of_days']}) for leave in leaves]))
        if 'leaves_to_process_count' in fields:
            ...
            if leaves_to_process:
                res.update(dict([(leave['employee_id'][0], {'leaves_to_process_count': leave['number_of_days']}) for leave in leaves_to_process]))
        return res
