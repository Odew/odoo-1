...
class WebsiteCrmPartnerAssign(WebsitePartnerPage):
    ...
    def partners(self, country=None, grade=None, page=0, **post):
        ...
        grades = Partner_sudo.read_group(grade_domain, ["id", "grade_id"], groupby="grade_id", orderby="grade_id DESC")
        # flag active grade
        ...
        countries = Partner_sudo.read_group(country_domain, ["id", "country_id"], groupby="country_id", orderby="country_id")
        # flag active country
        ...
        partner_count = Partner_sudo.search_count(base_partner_domain)
        pager = request.website.pager(
            url=url, total=partner_count, page=page, step=self._references_per_page, scope=7,
            url_args=url_args)

        # search partners matching current search parameters
        partners = Partner_sudo.search(base_partner_domain, order="grade_id DESC")  # todo in trunk: order="grade_id DESC, implemented_count DESC", offset=pager['offset'], limit=self._references_per_page
        # remove me in trunk
