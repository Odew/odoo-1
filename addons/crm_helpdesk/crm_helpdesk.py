...
class crm_helpdesk(osv.osv):
    ...
    def _get_attachment_number(self, cr, uid, ids, fields, args, context=None):
        ...
        for app_id in ids:
            res[app_id] = self.pool['ir.attachment'].search_count(cr, uid, [('res_model', '=', 'crm.helpdesk'), ('res_id', '=', app_id)], context=context)
        return res
    ...
    def _auto_init(self, cr, context=None):
        ...
        return self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table, super(crm_helpdesk, self)._auto_init,
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        helpdesk_id = super(crm_helpdesk, self).create(cr, uid, vals, context=create_context)
        helpdesk = self.browse(cr, uid, helpdesk_id, context=context)
        partner_id = vals.get('partner_id')
        if partner_id:
            self.pool.get('mail.alias').write(cr, uid, [helpdesk.alias_id.id], {'alias_parent_thread_id': helpdesk_id, 'alias_defaults': {'helpdesk_id': helpdesk_id}}, context=context)
            self.message_subscribe(cr, uid, [helpdesk_id], [partner_id], context=context)
        return helpdesk_id

    def unlink(self, cr, uid, ids, context=None):
        mail_alias = self.pool.get('mail.alias')
        alias_ids = [helpdesk.alias_id.id for helpdesk in self.browse(cr, uid, ids, context=context) if helpdesk.alias_id]
        res = super(crm_helpdesk, self).unlink(cr, uid, ids, context=context)
...
class crm_helpdesk_tag(osv.Model):
