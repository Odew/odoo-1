...
var AbstractManyField = common.AbstractField.extend({
    ...
    set_value: function(value_) {
        ...
        var ids = this.get_ids_from_command_values(value_);
        this._super(ids);
    ...
    get_ids_from_command_values: function (value_, command_list) {
        var self = this;
        var ids;
        if (command_list) {
            for (var i=0; i<command_list.length; i++) {
                command_list[i] = commands[command_list[i]];
            }
        }
        value_ = value_ || [];
        if(value_.length >= 1 && value_[0] instanceof Array) {
            ...
            _.each(value_, function(command) {
                ...
                if (commands && commands.length && command[0].indexOf(command_list) === -1) {
                    ids.push(obj);
                }
                switch (command[0]) {
...
var FieldMany2ManyCheckBoxes = AbstractManyField.extend(common.ReinitializeFieldMixin, {
    ...
    set_value: function(value_) {
        var value_ = this.get_ids_from_command_values(value_, ['LINK_TO', 'REPLACE_WITH']);
        var formatted = {};
