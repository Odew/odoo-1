...
class FleetVehicle(models.Model):
    ...
    def _compute_vehicle_name(self):
        if self.model_id:
            self.name = self.model_id.make_id.name + ' / ' + self.model_id.name + ' / ' \
                + self.license_plate
        else:
    ...
    def return_action_to_open(self):
        ...
        if self.env.context.get('xml_id'):
            ...
            result['context'] = dict(self.env.context, default_vehicle_id=self[0].id)
            result['domain'] = [('vehicle_id', '=', self[0].id)]
            return result
        return False
    ...
    def _compute_get_contract_reminder(self):
        ...
        for vehicle in self:
            ...
            total = 0
            name = ''
            for element in vehicle.log_contracts:
    ...
    def _compute_count_all(self):
        ...
        for vehicle_id in self:
            ...
            vehicle_id.cost_count = VehicleCost.search_count([('vehicle_id', '=', vehicle_id.id), ('parent_id', '=', False)])
...
class FleetVehicleLogContract(models.Model):
    ...
    def _compute_vehicle_contract_name_get(self):
        for log_contract in self:
            name = log_contract.vehicle_id.name if log_contract.vehicle_id else ''
            if log_contract.cost_subtype_id and log_contract.cost_subtype_id.name:
                name += ' / ' + log_contract.cost_subtype_id.name
            if log_contract.date:
                name += ' / ' + log_contract.date
    ...
    def act_renew_contract(self):
        self.ensure_one()
        if len(self.ids) > 1:
            raise UserError("This operation should only be done for 1 single contract at a time, "
