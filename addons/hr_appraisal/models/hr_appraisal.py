...
class HrEmployee(models.Model):
    ...
    def _appraisal_count(self):
        appraisals = self.env['hr.appraisal'].read_group([('employee_id', 'in', self.ids)], ['employee_id'], ['employee_id'])
        result = dict([(x['employee_id'][0], x['employee_id_count']) for x in appraisals])
