...
class Notification(models.Model):
    ...
    def update_message_notification(self, message_id, partner_ids):
        # update existing notifications
        self.write({'is_read': False})

        # create new notifications
        new_notif_ids = self.env['mail.notification']
        for new_pid in partner_ids - self.mapped('partner_id'):
            new_notif_ids |= self.create({'message_id': message_id.id, 'partner_id': new_pid.id, 'is_read': False})
    ...
    def _notify_email(self, message_id, force_send=False, user_signature=True):
        ...
        if message.model and message.res_id and self.pool.get(message.model) and hasattr(self.pool[message.model], 'message_get_email_values'):
            custom_values = self.pool[message.model].message_get_email_values(message.res_id, message)
        ...
        for chunk in chunks:
            ...
            email_ids.append(self.pool.get('mail.mail').create(mail_values))
        # NOTE:
        ...
        if force_send and len(chunks) < 2 and \
            ...
            self.pool.get('mail.mail').send(email_ids)
        return True

    @api.model
    def _notify(self, message_id, partners_to_notify=None, force_send=False, user_signature=True):
        ...
        """
        notif_ids = self.sudo().search([('message_id', '=', message_id.id), ('partner_id', 'in', partners_to_notify.ids)])

        # update or create notifications
        new_notif_ids = notif_ids.update_message_notification(message_id, partners_to_notify)  # tde check: sudo
        ...
        new_notif_ids._notify_email(message_id, force_send, user_signature)  # tde check this one too
