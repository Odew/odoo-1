...
class Message(models.Model):
    ...
    def _get_to_read(self):
        ...
        for notification in notifications:
            notification.message_id.to_read = True
    ...
    def set_message_starred(self, starred, create_missing=True):
        ...
        if len(notifications) != len(self.ids) and create_missing:
            for message in self - notifications.mapped('message_id'):
                self.env['mail.notification'].create(dict(values, partner_id=self.env.user.partner_id.id, message_id=message.id))
    ...
    def _message_read_dict(self, cr, uid, message, parent_id=False, context=None):
        ...
        try:
            if parent_id:
                ...
                max_length = 100
            body_short = tools.html_email_clean(message.body, remove=False, shorten=True, max_length=max_length)
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        res = super(Message, self).read(cr, uid, ids, fields=fields, context=context, load=load)
        return res
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(Message, self).unlink(cr, uid, ids, context=context)
    ...
    def _notify(self, force_send=False, user_signature=True):
        ...
        if self.subtype_id and self.model and self.res_id:
            ...
            partners_to_notify |= followers.filter(lambda fol: self.subtype_id in fol.subtype_ids).mapped('partner_ids')
        ...
        # notify
        self.env['mail.notification']._notify(self, partners_to_notify=partners_to_notify, force_send=force_send, user_signature=user_signature)
